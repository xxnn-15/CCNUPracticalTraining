<template>
    <div>
        <el-dialog
            v-model="dialogVisible_ForgetPassword"
            width="50%"
            @close="changeDialogVisible_ForgetPassword"
            center
        >
...
<script>
    ...
    export default {
        ...
        watch: {
            show(newVal) {
                this.dialogVisible_ForgetPassword = newVal //对父组件传过来的值进行监听，如果改变也对子组件内部的值进行改变
            }
        },

        methods: {
            changeDialogVisible_ForgetPassword() {
                this.$emit('changeDialogVisible_ForgetPassword')
            },
            ...
            sendEmail() {
                ...
                axios({
                    method:'post',
                    url: HostURL + 'api/sendVerificationCode_forgetPassword/',
                    data: Qs.stringify({
                        // 发送邮件，将 token 作为身份验证
                        email: this.email
                    })
            ...
            next() {
                if (this.stepActive==0) {
                    // 将用户输入的验证码发送到服务器，进行验证 这里直接使用注册的验证逻辑
                    axios({
                        method:'post',
                        ...
                        .then(
                            (res)=> {
                                ...
                                if (state == 'fail') {
                                    ...
                                    this.email = '' // 清空邮箱，以正确处理用户下一次请求
                                    this.changeDialogVisible_ForgetPassword()
